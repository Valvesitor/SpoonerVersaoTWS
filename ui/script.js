...
function ensureFavouriteBuckets() {
...
ensureFavouriteBuckets();

function normalizePedModels(list) {
	var seen = {};
	var normalized = [];
	...
	list.forEach(name => {
		...
		var model = name.trim().toLowerCase();
		if (!model || !/^[a-z0-9_]+$/.test(model) || seen[model]) {
			return;
		}
...
function parsePedModelsPayload(payload) {
...
function refreshPedMenus() {
	ensureFavouriteBuckets();

	if (document.getElementById('ped-list')) {
		populatePedList(document.getElementById('ped-search-filter').value);
	}

	if (document.getElementById('player-model-list')) {
...
function twsColorizeText(value) {
	var text = String(value || '');
	var parts = text.split(/([:@_\-\/\.\s\[\]\(\)]+)/g);
	var wordIndex = 0;
	var classes = ['tws-word-a', 'tws-word-b', 'tws-word-c', 'tws-word-d', 'tws-word-e'];
	...
	return parts.map(function(part) {
		...
		if (/^[:@_\-\/\.\s\[\]\(\)]+$/.test(part)) {
			return '<span class="' + (part.indexOf(':') !== -1 ? 'tws-colon' : 'tws-separator') + '">' + twsEscapeHtml(part) + '</span>';
		}

		var className = classes[wordIndex % classes.length];
		wordIndex++;
		return '<span class="tws-word ' + className + '">' + twsEscapeHtml(part) + '</span>';
	}).join('');
}

function twsColorizeObjectElement(element) {
	if (!element || element.nodeType !== 1 || element.hasAttribute('data-tws-colored')) {
		return;
	}

	if (!element.classList || !element.classList.contains('object')) {
		return;
	}

	var raw = element.getAttribute('data-tws-label') || element.textContent || '';
	raw = raw.trim();
	...
	element.setAttribute('data-tws-label', raw);
	element.setAttribute('title', raw);
	element.innerHTML = twsColorizeText(raw);
	element.setAttribute('data-tws-colored', '1');
}

function twsColorizeCurrentLists(root) {
	(root || document).querySelectorAll('.object').forEach(twsColorizeObjectElement);
}

function twsInstallElegantListObserver() {
	twsColorizeCurrentLists(document);
	...
	var observer = new MutationObserver(function(mutations) {
		mutations.forEach(function(mutation) {
			mutation.addedNodes.forEach(function(node) {
				...
				if (node.classList && node.classList.contains('object')) {
					twsColorizeObjectElement(node);
				}

				if (node.querySelectorAll) {
					twsColorizeCurrentLists(node);
				}
			});
		});
	});
...
function updateSpoonerHud(data) {
	...
	switch(data.adjustMode) {
		...
		case 2:
			...
			break;
		case 3:
			document.querySelector('#adjust-mode').innerHTML = 'Rodar';
			break;
		case 4:
			document.querySelector('#adjust-mode').innerHTML = 'Livre';
			break;
		case 5:
			document.querySelector('#adjust-mode').innerHTML = 'Desl.';
			break;
	...
	switch(data.rotateMode) {
		case 0:
			document.querySelector('#rotate-mode').innerHTML = 'Inclinar';
			break;
		case 1:
			document.querySelector('#rotate-mode').innerHTML = 'Girar';
			break;
		case 2:
			document.querySelector('#rotate-mode').innerHTML = 'Virar';
			break;
	}
	...
	if (data.placeOnGround) {
		document.querySelector('#place-on-ground').innerHTML = 'Lig.';
	} else {
		document.querySelector('#place-on-ground').innerHTML = 'Desli.';
	}
	...
	switch(data.entityType) {
		case 1:
			...
			break;
		case 2:
			document.querySelector('#entity-type').innerHTML = 'Veéculo';
			break;
		case 3:
			document.querySelector('#entity-type').innerHTML = 'Objeto';
			break;
			case 4:
				document.querySelector('#entity-type').innerHTML = 'Spooni';
				break;
		default:
			document.querySelector('#entity-type').innerHTML = 'Entidade';
			break;
	}
	...
	if (data.focusTarget) {
		...
		document.getElementById('focus-mode').innerHTML = data.freeFocus ? 'Livre' : 'Fixo';
		focusInfo.style.display = 'visível';
	} else {
		focusInfo.style.display = 'escondido';
	}
}
...
function populateSpooniList(filter) {
	...
	var favsOnly = document.getElementById('favourite-spooni').hasAttribute('data-active');

	spooniList.innerHTML = '';

	spooni.forEach(name => {
		var isFav = favourites.spooni[name];
		...
		if (!filter || filter == '' || name.toLowerCase().includes(filter.toLowerCase())) {
			...
			div.setAttribute('data-favourite-name', name);
			div.setAttribute('data-tws-label', name);
...
function openEntitySelect(menuId, onEntitySelect, ignoreEntity) {
	...
	entitySelectList.className = 'lista';
...
window.addEventListener('message', function(event) {
	switch (event.data.type) {
		...
		case 'updatePermissions':
			...
			break;
		case 'updatePedsList':
			peds = normalizePedModels(parsePedModelsPayload(event.data.peds));
			refreshPedMenus();
			break;
		case 'showControls':
			showControls();
...
window.addEventListener('load', function() {
	...
	sendMessage('init', {}).then(resp => resp.json()).then(function(resp) {
		...
		peds = normalizePedModels(parsePedModelsPayload(resp.peds));
		populatePedList();
