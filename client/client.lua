...
if Config.isRDR then
	...
	DetachPrompt = Uiprompt:new(`INPUT_INTERACT_LEAD_ANIMAL`, "Desanexar", SpoonerPrompts)
...
RegisterNetEvent('spooner:receiveServerPeds')

local function NormalizePedModelName(name)
...
local function MergeServerPeds(serverPeds)
	if type(serverPeds) ~= 'table' then return end
	if type(Peds) ~= 'table' then
		CreateThread(function()
			while type(Peds) ~= 'table' do
				Wait(100)
			end

			MergeServerPeds(serverPeds)
		end)
		return
	end
	...
	for _, pedName in ipairs(Peds) do
		local normalized = NormalizePedModelName(pedName)
		if normalized then
	...
	local added = 0
	for _, pedName in ipairs(serverPeds) do
		local normalized = NormalizePedModelName(pedName)
		if normalized and not seen[normalized] then
			seen[normalized] = true
			table.insert(Peds, normalized)
			added = added + 1
		end
	end
	...
	PedsHashLookup = GetIndexedHashList(Peds)
...
AddEventHandler('spooner:receiveServerPeds', MergeServerPeds)
...
AddEventHandler('spooner:init', function(permissions)
	...
	TriggerServerEvent('spooner:requestServerPeds')
end)
...
function ConvertDatabaseToDoorhash(database, content)
    local baseText = KeyboardInput("Introduza o nome base para os doorhashes:", "", 50)
...
function InitPrompts()
  CreatePromptButton('spooner', "Rodar" .. " -", 'INPUT_SELECT_NEXT_WEAPON')
  CreatePromptButton('spooner', "Rodar" .. " +", 'INPUT_SELECT_PREV_WEAPON')
  CreatePromptButton('spooner', "Cima", 'INPUT_FRONTEND_UP')
  CreatePromptButton('spooner', "Baixo", 'INPUT_FRONTEND_DOWN')
  CreatePromptButton('spooner',"Colocar", 'INPUT_FRONTEND_ACCEPT', 1500)
  CreatePromptButton('spooner',"Cancelar", 'INPUT_FRONTEND_CANCEL', 1500)
end
